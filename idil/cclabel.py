...
def run(img):
    ...
    for (x, y) in labels:
        ...
        if component not in componentPixel:
            componentPixel[component] = 1
        else:
            componentPixel[component] = 1 + componentPixel[component]
        # Colorize the image
...
def main(strr):
    # Open the image
    img = Image.open(strr)#sys.argv[1])
    pix = img.load()
    #img.show()
    ...
    return output_img
#if __name__ == "__main__": main(strr)
